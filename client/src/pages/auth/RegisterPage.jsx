...
export default function RegisterPage() {
  const [form, setForm] = useState({ name: '', email: '', password: '', college_id: '' })
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-gradient-to-br from-[#10101a] to-[#0a0a0f] items-center justify-center overflow-hidden">
        ...
        <div className="relative text-center px-12">
          ...
          <h2 className="text-2xl font-bold text-white mb-3">Join ClubSphere</h2>
          <p className="text-[#8888aa] text-sm leading-relaxed max-w-xs mx-auto">
            Create your account to discover college clubs, connect with communities, and make the most of campus life.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-2xl font-bold text-[#f0f0ff] mb-1">Create account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 mt-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-[#8888aa] mb-1.5">Full Name</label>
              <input
                id="name"
                type="text"
                required
            ...
            <div>
              <label htmlFor="reg-email" className="block text-sm font-medium text-[#8888aa] mb-1.5">Email</label>
              <input
                id="reg-email"
                type="email"
                required
            ...
            <div>
              <label htmlFor="college" className="block text-sm font-medium text-[#8888aa] mb-1.5">
                College
                <span className="text-[#555577] ml-1">(optional)</span>
              </label>
              {collegesLoading ? (
                <div className="flex items-center gap-2 h-10 px-3">
                  <Spinner size="sm" />
                  <span className="text-xs text-[#555577]">Loading colleges...</span>
                </div>
              ) : (
                <select
                  id="college"
