    ...
    <div className={`card p-5 bg-gradient-to-br ${style.gradient} border ${style.border} transition-all duration-200 hover:shadow-lg hover:shadow-black/20 flex flex-col gap-4`}>
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0">
          {club.logo_url ? (
            <img src={club.logo_url} alt={club.name} className="w-12 h-12 rounded-xl object-contain bg-white/5 border border-white/10 p-1" />
          ) : (
      ...
      <div className="flex flex-wrap gap-2 text-[10px] text-[#555577]">
        {club.membership_fee != null && (
          <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5 border border-white/5">
            ...
            {club.membership_fee === 0 ? 'Free' : `Rs. ${club.membership_fee}`}
          </span>
        )}
        ...
        {club.accepting_members != null && (
          <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full border ${club.accepting_members ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
...
function DeleteModal({ target, deleting, deleteError, onConfirm, onClose }) {
  return createPortal(
    <div
      ...
      onClick={onClose}
    >
      <div className="card w-full max-w-sm p-6 shadow-2xl border border-[#2a2a4a]" onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col items-center text-center mb-5">
          <div className="w-14 h-14 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-4">
            <AlertTriangle className="w-7 h-7 text-red-400" />
          </div>
          <p className="font-bold text-[#f0f0ff] text-base">Delete club?</p>
          <p className="text-sm text-[#8888aa] mt-2 leading-relaxed">
            Are you sure you want to permanently delete{' '}
            <span className="text-[#f0f0ff] font-semibold">{target.name}</span>?
    ...
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#8888aa] mt-0.5">Explore all clubs in your college</p>
        </div>
        <button
          id="request-club-btn"
      ...
      {!loading && clubs.length === 0 && !error && (
        <div className="text-center py-20">
          ...
          <p className="text-[#8888aa] text-sm mb-5">Be the first to request a club for your college!</p>
          <button onClick={() => navigate('/clubs/request')} className="btn-primary text-sm py-2 px-5 text-white">
            <Plus className="w-4 h-4" />
            Request a Club
          </button>
        </div>
