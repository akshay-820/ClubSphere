...
import { ArrowRight, Users, Building2, BookOpen, Rocket, Star, Zap } from 'lucide-react'
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#f0f0ff] flex flex-col animate-page-enter">
      ...
      <header className="relative z-10 border-b border-[#1e1e3a]/60 glass">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={logo} alt="ClubSphere" className="h-9 w-9 object-contain" />
            <span className="font-bold text-xl gradient-text">ClubSphere</span>
          </div>
          <div className="flex items-center gap-3">
            <Link
              to="/login"
      ...
      <section className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 pt-20 pb-16 text-center">
        ...
        <p className="text-lg text-[#8888aa] max-w-2xl mb-10 leading-relaxed">
          ClubSphere brings every college club, event, and student community onto one platform.
          Discover what&apos;s happening on campus, join clubs that excite you, and build something that lasts.
        </p>
        ...
        <div className="flex items-center gap-10 mt-16">
          {stats.map(s => (
            <div key={s.label} className="text-center">
              ...
              <p className="text-sm text-[#555577] mt-1">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-[#8888aa] max-w-xl mx-auto">
              One platform for students, clubs, and colleges to connect, collaborate, and grow.
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {features.map(f => (
              <div key={f.title} className={`card p-6 bg-gradient-to-br ${f.color} border-[#1e1e3a]`}>
                <div className={`w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center mb-4 ${f.iconColor}`}>
                  <f.icon className="w-5 h-5" />
                </div>
                <h3 className="font-semibold text-[#f0f0ff] mb-2">{f.title}</h3>
      ...
      <section className="relative z-10 py-16 px-6 text-center border-t border-[#1e1e3a]/60">
        ...
        <h2 className="text-2xl font-bold mb-3">Ready to find your people?</h2>
        <p className="text-[#8888aa] mb-7 text-sm">Join thousands of students already on ClubSphere.</p>
        <Link to="/register" className="btn-primary text-sm px-7 py-2.5 text-white">
      ...
      <footer className="relative z-10 border-t border-[#1e1e3a]/60 py-6 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between text-xs text-[#555577]">
          <div className="flex items-center gap-2">
            ...
            <span>&copy; {new Date().getFullYear()} ClubSphere</span>
          </div>
